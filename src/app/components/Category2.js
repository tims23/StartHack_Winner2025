...
const getMatchInformation = async (imLongitude, imLatitude, imCropName, imTargetMonth) =>
    new Promise(async(resolve, reject) => {
      
      const fieldData = await getFieldData(imLongitude, imLatitude, imTargetMonth);
      ...
      if (fieldData[0].temp_max <= cropData[0].temperature_optimum) {
        ...
        heatStress = (fieldData[0].temp_max - cropData[0].temperature_optimum) / (fieldData[0].temperature_max - cropData[0].temperature_optimum)
      }
      
      const minTemp = -20.0;
      const maxTemp = 60.0;
      const ratio_crop = 80 / (cropData[0].temperature_max - cropData[0].temperature_optimum);
      const offset_crop = (cropData[0].temperature_optimum - minTemp) / (maxTemp - minTemp) * 100;
      const offset_field = (fieldData[0].temp_avg - minTemp) / (maxTemp - minTemp) * 100;
    ...
    <div className="flex flex-col items-center justify-center transition-all duration-300">
      <div className="mb-6 mt-6">
        ...
        <div className={`transition-all duration-300 ${isDropdownOpen ? "mt-32" : ""}`}>
          <h5 className="text-md font-semibold mb-4 text-green-300">Heat Stress</h5>
        ...
        >Field Temperature: {matchInformation.field_temp_avg}°</div>
        </div>
        </div>
        <p>
            Heat Stress: {matchInformation.heatStress}
        </p>
      </div>
    </div>
  );
